...
class Board {
    ...
    PieceState getWinner() {
        ...
        int redCount = getColorNums(RED);
        int blueCount = getColorNums(BLUE);
        winner = null;
        if (getConsecJumpNums() >= CONSEC_JUMP_LIMIT || (!couldMove(RED) && !couldMove(BLUE)) || redCount == 0 || blueCount == 0) {
            if (redCount == blueCount) {
                winner = EMPTY;
            } else {
                winner = redCount > blueCount ? RED : BLUE;
            }
        }
